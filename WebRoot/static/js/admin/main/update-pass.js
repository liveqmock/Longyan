...
$(document).ready(function(){
	var Page = {
		...
		_init: function(){
			...
			me.jQupdate = $('#update');
			me.jQcancel = $('#cancel');
			me.jQmodifyPass = $('.modify-pass');
			me.jQpassWrapPop = $('.pass-wrap-pop');
		...
		_initEvent: function(){
			...
			me.jQpassTable.on('blur', '.pass-required', function(e){
			...
			me.jQmodifyPass.on('click', function(){
				me._pop();
			});
		...
		_commit: function(tar){
			...
			$.ajax({
				url: '/Longyan/admin/filter/update-employee-password',
				data: {
				...
				if(json){
					if(json.code == 2001){   //通过登录验证
						alert('修改成功，请重新登录');
						window.location.href = '/Longyan/admin/login';
		...
		_pop: function(){
			var me = this;
			me.jQpassWrapPop.show();
		},
		_hide: function(){
			var me = this;
			me.jQpassWrapPop.hide();
		}
	};
